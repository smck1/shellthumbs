...
int wmain(int argc, wchar_t *argv[]) {
	{
		...
		// Do somesthing stupid to get the path.
		wchar_t filepath[MAX_PATH] = { 0 };
		int arg_len = wcslen(argv[1]);
		wmemcpy_s(filepath, MAX_PATH, argv[1], (arg_len > MAX_PATH ? MAX_PATH : arg_len));
		...
		if (hFile != INVALID_HANDLE_VALUE)
		{
			...
			ReadFile(hFile, data_buffer, size, &read, NULL);
			CloseHandle(hFile);
